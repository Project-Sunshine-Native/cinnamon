...
static void gsInit(Renderer* renderer, DataWin* dataWin) {
    ...
    printf("GsRenderer: initialized (colored quads mode, no textures)\n");
    printf("GsRenderer: %u sprites, %u TPAG items\n", dataWin->sprt.count, dataWin->tpag.count);
}
...
static void gsBeginView(Renderer* renderer, int32_t viewX, int32_t viewY, int32_t viewW, int32_t viewH, [[maybe_unused]] int32_t portX, [[maybe_unused]] int32_t portY, int32_t portW, int32_t portH, [[maybe_unused]] float viewAngle) {
    ...
    // Use port size as the target area, mapped to 640x448
    if (viewW > 0 && viewH > 0) {
        // Use uniform scale based on width (640/viewW) so pixels stay square.
        // For Undertale's 320x240: scale = 2.0, rendered height = 480, but screen is 448,
        // so 16px are cropped from top and 16px from bottom (centered vertically).
    ...
    // renderedH = viewH * scaleY, offsetY = (448 - renderedH) / 2 (negative = crops top/bottom)
