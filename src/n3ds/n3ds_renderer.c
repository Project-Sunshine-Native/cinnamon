...
static void CBeginFrame(Renderer* renderer, u32 clearColor, uint32_t speed, int32_t gameW, int32_t gameH, int32_t windowW, int32_t windowH)
{
    ...
    CRenderer3DS* C = (CRenderer3DS*) renderer;
    C->zCounter = 0.5f;
    // Store game/screen dimensions so CBeginView can compose the two-level transform.
    // Do NOT call CBeginView here - the game engine calls it separately each frame
    // with the actual camera coords (viewX/viewY).  Calling it here with viewX=0
    // would be overridden anyway and served no purpose.
    C->gameW   = gameW;   C->gameH   = gameH;
    C->screenW = windowW; C->screenH = windowH;
    C3D_FrameBegin(C3D_FRAME_SYNCDRAW);
